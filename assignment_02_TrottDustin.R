...
person_df2 <- read.csv("data/person.csv", stringsAsFactors = FALSE)
str(person_df2)
...
## I had to use this command to first load the library because it said it was not found: install.packages("readxl")
library("readxl")
...
voter_turnout_df2 <- read_excel("data/G04ResultsDetail2004-11-02.xls", sheet = "Voter Turnout", skip = 2, col_names = "ward_precint", "ballots_cast", "registered_voters", "voter_turnout")
str(voter_turnout_df2)

## Load the `DBI` library
# had to run this to install the DBI packagage first: install.packages("DBI")
library(DBI)
...
## had to run the following command to install RSQLite: install.packages("RSQLite")
db <- dbConnect(RSQLite::SLite(), data/tidynomicon/example.db)
